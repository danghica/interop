...
The design of `Extern` is inspired by that of `dynamic` in C#. 
The difference is that the restrictions in binding and assignment discussed below will make it difficult to use `Extern` as an escape hatch from the type system, avoiding the dark patterns that `dynamic` is sometimes used for. 
...
Consider `extexp` an expression that has `Extern` type and `cjexp` an expression that has a Cangjie type `T`. 
