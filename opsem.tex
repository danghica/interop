...
\begin{itemize}
  ...
  $e \Coloneqq () \mid \textsf{true} \mid \textsf{false} \mid i \mid x \mid \lambda x{:}\tau.\, e \mid e\; e \mid (e_1, e_2) \mid \text{fst}\; e \mid \text{snd}\; e \mid \text{if}\; e\ \text{then}\; e\ \text{else}\; e \mid \text{ref}\; e \mid {!}e \mid e \mathrel{:=} e \mid e \mathrel{:=}^{\textsf{ext}}_{\tau} e \mid \text{cast}_{\tau}\, e \mid \phi \mid \text{let}\; x{:}\tau = e\ \text{in}\; e$
  \item \textbf{Values:}
...
A pair $(\delta, \delta^{\text{ext}})$ is \emph{extern-compatible}, written $\delta \sim_{\textsf{ext}} \delta^{\text{ext}}$, when $\delta^{\text{ext}}$ has the same data shape as $\delta$ and may replace any native leaf of $\delta$ by \textsf{extern}. By construction, extern-compatible types are data-only: function types $\tau_1 \to \tau_2$ are excluded. 
...
References of non-native data types are not inhabited, so the dereference and assignment rules are trivially applicable to such types. Therefore, we do not need to distinguish between native and non-native data types in these rules.
...
\begin{gather*}
  ...
  \\[0.75em]
  \frac{\Gamma \vdash e_1 : \tau\,\text{ref} \quad \Gamma \vdash e_2 : \tau' \quad \tau \sim_{\textsf{ext}} \tau'}{\Gamma \vdash e_1 \mathrel{:=}^{\textsf{ext}}_{\tau} e_2 : \text{unit}}\,\text{[{Assign-Extern}]}
  \\[0.75em]
  \frac{}{\Gamma \vdash \phi : \tau\rightarrow\text{extern}}\,\text{[{Foreign-Const}]}
\end{gather*}

Creating  a reference of a non-native type is disallowed via the type system.
The assignment operation is only applicable to references of native types, but the assigned values can be non-native and extern-compatible. The assignent operation is semantically a different operation than the assignment operation for native types because it will insert a runtime type check to ensure that the assigned value is compatible with the expected type.

Extern typed values can only be produced by foreign functions. They can also be bound to function arguments called by functions in the non-native code.

\begin{gather*}
  \frac{\Gamma \vdash e_1 : \tau_1 \quad \Gamma, x{:}\tau_1 \vdash e_2 : \tau_2 \quad \text{$\tau_1$ is a native type}}{\Gamma \vdash \text{let}\; x{:}\tau_1 = e_1\ \text{in}\; e_2 : \tau_2}\,\text{[{Let-Native}]}
  \\[0.75em]
  \frac{\Gamma \vdash e_1 : \tau_1^{\text{ext}} \quad \tau_1^{\text{ext}} \sim_{\textsf{ext}} \tau_1 \quad \Gamma, x{:}\tau_1 \vdash e_2 : \tau_2 \quad \text{$\tau_1$ is a native type}}{\Gamma \vdash \text{let}\; x{:}\tau_1 = e_1\ \text{in}\; e_2 : \tau_2}\,\text{[{Let-Extern-Compatible}]}
\end{gather*}
...
\[
  \frac{\Gamma \vdash e_1 : \tau_1 \to \tau_2 \quad \Gamma \vdash e_2 : \tau_1' \quad \tau_1' \sim_{\textsf{ext}} \tau_1}{\Gamma \vdash e_1\; e_2 : \tau_2}\,\text{[App-Extern-Compatible]}
\]

\textbf{Note:} The App-Extern-Compatible rule is a generalization of the App rule that allows for the application of a function to an extern-compatible value, since any native type is trivially extern-compatible to itself.
\[
  \frac{\Gamma \vdash e : \tau' \quad \tau' \sim_{\textsf{ext}} \tau}{\Gamma \vdash \text{cast}_{\tau}\, e : \tau}\,\text{[Cast-Extern-Compatible]}
\]
The extern-compatible typing rules (\textbf{[Let-Extern-Compatible]}, \textbf{[App-Extern-Compatible]}, \textbf{[Assign-Extern]}) are understood as inserting these explicit boundary casts toward the expected native type.

\section{Operational Semantics ($\langle e, \mu \rangle \to \langle e', \mu' \rangle$)}

We take $e$ to be the expression and $\mu$ the store (mapping locations $l$ to values $v$). Runtime cell-content types are not stored in a separate component.
Instead, boundary conversions are made explicit by $\text{cast}_{\tau}\,e$, where $\tau$ is a native expected type inserted from typing (for \texttt{let}, extern-compatible application, and extern-aware assignment).

\subsection{Administrative Reductions}
\[
\begin{aligned}
  \langle (\lambda x{:}\tau.\, e)\ v, \mu \rangle &\to \langle e[v/x], \mu \rangle \quad (\text{if } v \text{ is already native at } \tau) \\
  \langle (\lambda x{:}\tau.\, e)\ \text{box}(v, \sigma), \mu \rangle &\to \langle (\lambda x{:}\tau.\, e)\ \text{cast}_{\tau}\, \text{box}(v, \sigma), \mu \rangle \\
  ...
  \langle l \mathrel{:=} v, \mu \rangle &\to \langle (), \mu[l \mapsto v] \rangle \quad (v \neq \text{box}(\ldots)) \\
  \langle l \mathrel{:=}^{\textsf{ext}}_{\tau} v, \mu \rangle &\to \langle l \mathrel{:=} \text{cast}_{\tau}\, v, \mu \rangle \\
...
Assignments are split into \textbf{[Assign]} (ordinary native write) and \textbf{[Assign-Extern]} (which first inserts $\text{cast}_{\tau}$ using the expected left-hand-side type).
...
Define a partial conversion relation on values, $v \Downarrow_{\tau} v'$.
The phrase ``conversion succeeds'' means exactly that a judgment $v \Downarrow_{\tau} v'$ is derivable from the following cases:
\[
\begin{gathered}
  \frac{\text{$\tau$ is a native type}}{v \Downarrow_{\tau} v}
  \\[0.75em]
  \frac{v_1 \Downarrow_{\tau_1} v_1' \quad v_2 \Downarrow_{\tau_2} v_2'}{(v_1, v_2) \Downarrow_{\tau_1 \times \tau_2} (v_1', v_2')}
  \\[0.75em]
  \frac{v \Downarrow_{\tau} v'}{\text{box}(v, \tau) \Downarrow_{\tau} v'}
\end{gathered}
\]
If no such derivation exists, conversion fails.
...
Let reduction rules:
\begin{gather*}
  \frac{}{ \langle \text{let}\; x{:}\tau = v\ \text{in}\; e_2, \mu \rangle \to \langle e_2[v/x], \mu \rangle}\,\text{[R-Let-Val]}
  \\[0.75em]
...
The typing rules \textbf{[Let-Extern-Compatible]}, \textbf{[App-Extern-Compatible]}, and \textbf{[Assign-Extern]} are interpreted by elaboration into explicit casts:
$\text{let}\; x{:}\tau = \text{cast}_{\tau}(e_1)\ \text{in}\; e_2$,
$e_1\;(\text{cast}_{\tau_1}(e_2))$,
and $e_1 := \text{cast}_{\tau}(e_2)$, respectively.
...
Following work on \emph{contracts} and \emph{gradual typing}, \emph{blame} names that responsibility: each runtime check is a contract between a \emph{native} expectation (the annotation $\tau$ in \texttt{let}, or the expected type carried by $\mathrel{:=}^{\textsf{ext}}_{\tau}$) and a \emph{foreign} or producer-side claim carried by the tag $\sigma$ on $\text{box}(v, \sigma)$.
...
\begin{itemize}
  ...
  \item \textbf{Extern assignment} cast failure: $\mathrel{:=}^{\textsf{ext}}_{\tau}$ inserts $\text{cast}_{\tau}$ before writing. If that cast fails, mismatch usually \emph{blames the writer} of the incompatible external value (or the logic that chose this target type $\tau$).
\end{itemize}
...
The note in the static section about inserting checks on $op(e)$ when $e:\textsf{extern}$ would be another boundary at which a full account could attach blame to the caller or callee of $op$, analogously to higher-order contracts.
Conceptually, blame refines \emph{who} failed, not \emph{when}: the failures above still arise at the first reduction step where the offending \texttt{let} or extern assignment runs, not after unbounded deferred checking.
...
\paragraph{Example 2 (blame at extern assignment).}
Suppose a reference cell is written through an extern-aware assignment expecting integers:
\[
  \langle l \mathrel{:=}^{\textsf{ext}}_{\text{int}} \text{box}(\textsf{false}, \text{bool}), \mu \rangle \to \text{error}.
\]
Again $\sigma=\text{bool}$ and $\tau=\text{int}$ disagree.
The usual reading blames the writer of the incompatible boxed value (or the logic that selected target type $\text{int}$ for this write).
...
Rule~\textsf{[Let]} requires the annotation $\tau_1$ to be a \emph{native} type, so one cannot annotate the bound variable with \textsf{extern}: a \texttt{let} of the form ``\texttt{let} $x:\text{extern} = e_1$ \texttt{in} $e_2$'' is not derivable, and one cannot ``fix'' a \texttt{let} by widening the bound name's type to \textsf{extern}.
The disjunct $\Gamma \vdash e_1 : \text{extern}$ in \textsf{[Let]} means the \emph{right-hand side} may arrive from the FFI as \textsf{extern} while $x$ is still given a \emph{native} type $\tau_1$---that is controlled interoperability, not replacement of $\tau_1$ by \textsf{extern}.
...
Rule~\textsf{[Abs]} does not require the parameter type $\tau_1$ to be native, whereas \textsf{[Let]} and \textsf{[Ref]} do constrain annotations and cell content types.
...
Rules \textsf{[Let]} and \textsf{[Ref]} keep storage and let-bound names on native types where the design intends a fixed ML shape; \textsf{[Abs]} stays more liberal precisely to model callbacks and other ``native code called with foreign arguments'' scenarios.
...
The \textsf{extern} boundary adds controlled casts at explicit FFI points; it does not turn the core into a flow-sensitive or dynamically typed regime.
...
Rule~\textsf{[Let]} is the delicate case for inference.
It concludes $\Gamma \vdash \texttt{let}\; x{:}\tau_1 = e_1\ \texttt{in}\; e_2 : \tau_2$ when (among other side conditions) either $\Gamma \vdash e_1 : \tau_1$ or $\Gamma \vdash e_1 : \text{extern}$, with $\tau_1$ a \emph{native} type.

If the implementer drops the annotation $\tau_1$ and tries to infer it \emph{only} from $e_1$, the situation is underdetermined: from $e_1$ alone one may learn little more than $\Gamma \vdash e_1 : \text{extern}$, which does not fix which native $\tau_1$ the programmer intends for $x$.
Conversely, inferring $\tau_1$ \emph{only} from uses of $x$ in $e_2$ ignores the possibility that $e_1$ is \textsf{extern} and must be cast at runtime to that $\tau_1$.
There is no subtyping rule making $\textsf{extern}$ a subtype of $\tau_1$ in this calculus, so one cannot unify $\textsf{extern}$ with arbitrary native $\tau_1$ by subsumption.

The standard way to recover a principal, predictable algorithm is therefore:
\begin{itemize}
  \item treat $\tau_1$ on the FFI \texttt{let} as \emph{required} (or generated from a surface \texttt{cast} form with an explicit target type), and
  \item \emph{check} $e_1$ against the disjunction $\Gamma \vdash e_1 : \tau_1 \lor \Gamma \vdash e_1 : \text{extern}$,
\end{itemize}
while continuing to \emph{infer} inside $e_2$ as in ordinary ML once $x : \tau_1$ is fixed.
Equivalently, one adopts \emph{bidirectional} typing at the boundary: the annotation $\tau_1$ is an input (check mode) for $e_1$, and an output (infer mode) for occurrences of $x$ in $e_2$.
...
In the core fragment without the \textsf{[Let]} disjunct, Hindley--Milner-style inference often yields \emph{principal} types: a most general typing from which all others follow by substitution.
At the FFI \texttt{let}, the disjunction $\Gamma \vdash e_1 : \tau_1 \lor \Gamma \vdash e_1 : \text{extern}$ means that several native choices of $\tau_1$ can be consistent with different uses unless an annotation fixes $\tau_1$.
