...
The rule for [App] is later on generalized and can be ommited in the case of native types.
...
References of non-native data types are not inhabited, so the dereference and assignment rules are trivially applicable to such types. Therefore, we do not need to distinguish between native and non-native data types in these rules. The assignment rule is later generalized and can be ommited in the case of native types.
...
\begin{gather*}
  ...
  \\[0.75em]
  \frac{\Gamma \vdash e_1 : \tau\,\text{ref} \quad \Gamma \vdash e_2 : \tau' \quad \tau \sim_{\textsf{ext}} \tau'}{\Gamma \vdash e_1 \mathrel{:=} e_2 : \text{unit}}\,\text{[{Assign-Extern}]}
\end{gather*}
...
\begin{gather*}
  \frac{\Gamma \vdash e_1 : \tau_1' \quad \tau_1' \sim_{\textsf{ext}} \tau_1 \quad \Gamma, x{:}\tau_1 \vdash e_2 : \tau_2 \quad \text{$\tau_1$ is a native type}}{\Gamma \vdash \text{let}\; x{:}\tau_1 = e_1\ \text{in}\; e_2 : \tau_2}\,\text{[{Let-Extern-Compatible}]}
\end{gather*}
...
The App-Extern-Compatible rule is a generalization of the App rule that allows for the application of a function to an extern-compatible value, since any native type is trivially extern-compatible to itself.
\[
...
\begin{aligned}
  ...
  \langle l \mathrel{:=} v, \mu \rangle &\to \langle (), \mu[l \mapsto v'] \rangle \quad (\text{if } v \Downarrow v') \\
...
Note that in application and in assigment, if the argument is of a native type the cast is trivial and can be omitted. 
...
The boundary forms $\text{let}^{\textsf{ext}}$ and $(\cdot)^{\textsf{ext}}$ in function application, together with \textbf{[Assign-Extern]}, are interpreted by elaboration into explicit casts:
$\text{let}\; x{:}\tau = \text{cast}_{\tau}(e_1)\ \text{in}\; e_2$ for $\text{let}^{\textsf{ext}}\; x{:}\tau = e_1\ \text{in}\; e_2$,
$e_1\;(\text{cast}_{\tau_1}(e_2))$ for $e_1\,(e_2)^{\textsf{ext}}$,
and $e_1 := \text{cast}_{\tau}(e_2)$ for $e_1 \mathrel{:=}^{\textsf{ext}}_{\tau} e_2$.
